...
export type UserRole = 'SUPER_ADMIN' | 'COMPANY_ADMIN' | 'DISPATCHER' | 'KITCHEN' | 'MOZO' | 'DRIVER' | 'CLIENT';
...
function createInitialSeedData(): DatabaseSchema {
  ...
  if (!adminPassword) {
    return {
      version: 2, companies: [], users: [], drivers: [], deliveries: [], location_sessions: [], events: [], driver_locations: [],
      food_stores: [], food_categories: [], food_products: [], food_shipping_rates: [], food_orders: [], invitations: [], password_resets: [],
    };
  }
  const salt = bcrypt.genSaltSync(10);
  const adminPasswordHash = bcrypt.hashSync(adminPassword, salt);
  const initialCompany = {
    id: 'comp_default_admin', name: 'Sede Central', category: 'Mensajería y Cadetería' as const, address: 'Ubicación Principal', phone: '', city: '',
    activeOrdersCount: 0, totalDriversCount: 0, businessType: 'LOGISTICS' as const, foodEnabled: false,
  };
  const initialAdmin = {
    id: 'usr_admin_master', email: 'admin@ubika.local', passwordHash: adminPasswordHash, name: 'Administrador Principal', role: 'SUPER_ADMIN' as const,
    companyId: 'comp_default_admin', createdAt: Date.now(), active: true,
  };
  return {
    version: 2, companies: [initialCompany], users: [initialAdmin], drivers: [], deliveries: [], location_sessions: [], events: [], driver_locations: [],
    food_stores: [], food_categories: [], food_products: [], food_shipping_rates: [], food_orders: [], invitations: [], password_resets: [],
  };
}
...
export function injectTestFixtures() {
  ...
  if (!db.companies.find((c: any) => c.id === 'comp_ubika_piloto')) { db.companies.push({ id: 'comp_ubika_piloto', name: 'UBIKA PILOTO', category: 'Mensajería y Cadetería', address: '', phone: '', city: '', activeOrdersCount: 1, totalDriversCount: 1, businessType: 'LOGISTICS', foodEnabled: false }); changed = true; }
  if (!db.companies.find((c: any) => c.id === 'comp_centro_logistico_01')) { db.companies.push({ id: 'comp_centro_logistico_01', name: 'Logística Express Centro', category: 'Mensajería y Cadetería', address: '', phone: '', city: '', activeOrdersCount: 2, totalDriversCount: 4, businessType: 'LOGISTICS', foodEnabled: false }); changed = true; }
  if (!db.companies.find((c: any) => c.id === 'comp_farma_norte_02')) { db.companies.push({ id: 'comp_farma_norte_02', name: 'Farmacia Norte', category: 'Farmacia / Salud', address: '', phone: '', city: '', activeOrdersCount: 0, totalDriversCount: 1, businessType: 'LOGISTICS', foodEnabled: false }); changed = true; }
  if (!db.companies.find((c: any) => c.id === 'comp_food_don_pedro_01')) { db.companies.push({ id: 'comp_food_don_pedro_01', name: 'Hamburguesería Don Pedro' as any, address: '', phone: '', city: '', activeOrdersCount: 0, totalDriversCount: 0, businessType: 'FOOD', foodEnabled: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_admin_piloto')) { db.users.push({ id: 'usr_admin_piloto', email: 'admin@ubikapiloto.com', passwordHash: adminPasswordHash, name: 'ADMIN PILOTO', role: 'COMPANY_ADMIN', companyId: 'comp_ubika_piloto', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_driver_piloto')) { db.users.push({ id: 'usr_driver_piloto', email: 'driver@ubikapiloto.com', passwordHash: driverPasswordHash, name: 'DRIVER PILOTO', role: 'DRIVER', companyId: 'comp_ubika_piloto', driverId: 'drv_piloto', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_don_pedro_01')) { db.users.push({ id: 'usr_don_pedro_01', email: 'donpedro@ubikafood.com', passwordHash: adminPasswordHash, name: 'Admin Don Pedro', role: 'COMPANY_ADMIN', companyId: 'comp_food_don_pedro_01', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_cocina_don_pedro_01')) { db.users.push({ id: 'usr_cocina_don_pedro_01', email: 'cocina@ubikafood.com', passwordHash: adminPasswordHash, name: 'Cocina Don Pedro', role: 'KITCHEN', companyId: 'comp_food_don_pedro_01', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_admin_01')) { db.users.push({ id: 'usr_admin_01', email: 'admin@logisticaexpress.com', passwordHash: adminPasswordHash, name: 'Martín Rodríguez', role: 'COMPANY_ADMIN', companyId: 'comp_centro_logistico_01', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.users.find((u: any) => u.id === 'usr_dispatcher_01')) { db.users.push({ id: 'usr_dispatcher_01', email: 'despacho@logisticaexpress.com', passwordHash: adminPasswordHash, name: 'Ana Gómez', role: 'DISPATCHER', companyId: 'comp_centro_logistico_01', createdAt: Date.now(), active: true }); changed = true; }
  if (!db.drivers.find((d: any) => d.id === 'drv_piloto')) { db.drivers.push({ id: 'drv_piloto', companyId: 'comp_ubika_piloto', name: 'DRIVER PILOTO', email: 'driver@ubikapiloto.com', phone: '', vehicle: 'moto', status: 'disponible', internalId: 'PILOTO-01', createdAt: Date.now(), totalDeliveries: 0, rating: 5, lastActiveAt: Date.now(), speedKmH: 0 }); changed = true; }
  if (!db.drivers.find((d: any) => d.id === 'drv_farma_01')) { db.drivers.push({ id: 'drv_farma_01', companyId: 'comp_farma_norte_02', name: 'Esteban Morales', email: 'esteban@farmanorte.com', phone: '', vehicle: 'moto', status: 'disponible', internalId: 'F-01', createdAt: Date.now(), totalDeliveries: 10, rating: 4.8, lastActiveAt: Date.now(), speedKmH: 0 }); changed = true; }
  if (!db.drivers.find((d: any) => d.id === 'drv_don_pedro_01')) { db.drivers.push({ id: 'drv_don_pedro_01', companyId: 'comp_food_don_pedro_01', name: 'Cadete Pedro Jr', email: 'pedrojr@ubikafood.com', phone: '', vehicle: 'moto', status: 'disponible', internalId: 'DP-01', createdAt: Date.now(), totalDeliveries: 15, rating: 5.0, lastActiveAt: Date.now(), speedKmH: 0 }); changed = true; }
...
export function loadDatabase(): DatabaseSchema {
  ...
  if (fs.existsSync(DB_FILE)) {
    let raw: string;
    try { raw = fs.readFileSync(DB_FILE, 'utf-8'); } catch (readErr: any) { throw new Error(`FATAL_DB_READ_ERROR: Archivo existente en ${DB_FILE} no puede ser leído: ${readErr.message}`); }
    try { dbState = JSON.parse(raw); } catch { throw new Error(`FATAL_DB_CORRUPTED: El archivo existente en ${DB_FILE} está corrupto. Inicialización detenida para preservar datos.`); }
    ...
    dbState.food_stores = dbState.food_stores || []; dbState.food_categories = dbState.food_categories || []; dbState.food_products = dbState.food_products || []; dbState.food_shipping_rates = dbState.food_shipping_rates || []; dbState.food_orders = dbState.food_orders || [];
    dbState.invitations = dbState.invitations || []; dbState.password_resets = dbState.password_resets || []; dbState.commerce_categories = dbState.commerce_categories || []; dbState.commerce_products = dbState.commerce_products || []; dbState.commerce_customers = dbState.commerce_customers || []; dbState.commerce_stock_movements = dbState.commerce_stock_movements || []; dbState.commerce_cash_sessions = dbState.commerce_cash_sessions || []; dbState.commerce_sales = dbState.commerce_sales || []; dbState.commerce_invoices = dbState.commerce_invoices || [];
    const migrationChanged = runFoodMigration(dbState); if (migrationChanged) saveDatabaseSync();
    console.log(`[DB] Base de datos persistente cargada con éxito desde ${DB_FILE}`); return dbState;
  }
  dbState = createInitialSeedData(); saveDatabaseSync(); console.log(`[DB] Archivo de persistencia inicial creado en ${DB_FILE}`); return dbState;
}
...
export const db = {
  ...
  getInvitationByHash: (hash: string): InvitationRecord | undefined => { dbState.invitations = dbState.invitations || []; return dbState.invitations.find((i) => i.tokenHash === hash); },
  updateInvitation: (id: string, updates: Partial<InvitationRecord>): InvitationRecord | null => { dbState.invitations = dbState.invitations || []; const idx = dbState.invitations.findIndex((i) => i.id === id); if (idx === -1) return null; dbState.invitations[idx] = { ...dbState.invitations[idx], ...updates }; saveDatabaseSync(); return dbState.invitations[idx]; },
  ...
  getPasswordResetByHash: (hash: string): PasswordResetRecord | undefined => { dbState.password_resets = dbState.password_resets || []; return dbState.password_resets.find((p) => p.tokenHash === hash); },
  updatePasswordReset: (id: string, updates: Partial<PasswordResetRecord>): PasswordResetRecord | null => { dbState.password_resets = dbState.password_resets || []; const idx = dbState.password_resets.findIndex((p) => p.id === id); if (idx === -1) return null; dbState.password_resets[idx] = { ...dbState.password_resets[idx], ...updates }; saveDatabaseSync(); return dbState.password_resets[idx]; },
  ...
  getSessionByToken: (token: string): LocationSessionRecord | undefined => { const hash = hashToken(token); return dbState.location_sessions.find((s) => s.sessionTokenHash === hash); },
  ...
  getFoodCategoriesByCompanyId: (companyId: string): FoodCategory[] => (dbState.food_categories || []).filter((c) => c.companyId === companyId).sort((a, b) => ((a.displayOrder ?? a.sortOrder ?? 999) - (b.displayOrder ?? b.sortOrder ?? 999)) || ((a.createdAt || 0) - (b.createdAt || 0))),
  ...
  getFoodOrderByPublicToken: (token: string): FoodOrder | undefined => (dbState.food_orders || []).find((o) => o.publicTrackingToken === token),
