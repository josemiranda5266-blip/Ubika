...
export function injectTestFixtures() {
  ...
  if (!db.food_stores.find(s => s.companyId === 'comp_food_don_pedro_01')) { db.food_stores.push({ companyId: 'comp_food_don_pedro_01', name: 'Hamburguesería Don Pedro', description: 'Burgers', address: 'Av Belgrano', phone: '', whatsappNumber: '', isOpenManual: true as any }); changed = true; }
  db.food_categories = db.food_categories || [];
  if (!db.food_categories.find(c => c.id === 'cat_burgers')) { db.food_categories.push({ id: 'cat_burgers', companyId: 'comp_food_don_pedro_01', name: 'HAMBURGUESAS', description: '', displayOrder: 1, active: true }); changed = true; }
  db.food_products = db.food_products || [];
  if (!db.food_products.find(p => p.id === 'prod_burg_clasica')) { db.food_products.push({ id: 'prod_burg_clasica', companyId: 'comp_food_don_pedro_01', categoryId: 'cat_burgers', name: 'Hamburguesa Clásica', description: 'Carne, queso', price: 8000, isAvailable: true, displayOrder: 1, imageUrl: '' }); changed = true; }
  db.food_shipping_rates = db.food_shipping_rates || [];
  if (!db.food_shipping_rates.find(r => r.companyId === 'comp_food_don_pedro_01')) { db.food_shipping_rates.push({ companyId: 'comp_food_don_pedro_01', baseFee: 1500, perKmFee: 200, includedKm: 0, maxDistanceKm: 10, storeLatitude: -34.6, storeLongitude: -58.4 }); changed = true; }
  db.food_orders = db.food_orders || [];
  if (!db.food_orders.find(o => o.orderNumber === 1075)) { db.food_orders.push({ id: 'forder_1075_dp_seed', orderNumber: 1075, companyId: 'comp_food_don_pedro_01', deliveryType: 'FOOD_PICKUP', items: [{ productId: 'prod_burg_clasica', productName: 'Hamburguesa Clásica', quantity: 1, unitPrice: 8000, selectedOptions: [], totalPrice: 8000 }], subtotal: 8000, shippingCost: 0, totalAmount: 8000, recipientName: 'Lucas Morales', recipientPhone: '+54 9 11 5555-1075', paymentMethod: 'CASH', paymentStatus: 'PENDING', pickupCode: 'DP107', publicTrackingToken: 'tr_food_demo_1075_don_pedro', orderStatus: 'PENDING', createdAt: Date.now() - 3600000, updatedAt: Date.now() - 3600000 }); changed = true; }
...
export function loadDatabase(): DatabaseSchema {
  ...
  if (fs.existsSync(DB_FILE)) {
    ...
    try { dbState = JSON.parse(raw); } catch (_parseErr: any) { throw new Error(`FATAL_DB_CORRUPTED: El archivo existente en ${DB_FILE} está corrupto. Inicialización detenida para preservar datos.`); }
    ...
    if (migrationChanged) saveDatabaseSync();
    return dbState;
  }
...
export const db = {
  ...
  createUser: (user: UserRecordInput): UserRecord => { const createdAt = user.createdAt || Date.now(); const normalized: UserRecord = { ...user, privacyPolicyAccepted: user.privacyPolicyAccepted ?? true, privacyPolicyAcceptedAt: user.privacyPolicyAcceptedAt ?? createdAt, termsOfServiceAccepted: user.termsOfServiceAccepted ?? true, termsOfServiceAcceptedAt: user.termsOfServiceAcceptedAt ?? createdAt }; dbState.users.push(normalized); saveDatabaseSync(); return normalized; },
  ...
  recordDriverLocation: (record: { driverId: string; deliveryId?: string; companyId: string; latitude: number; longitude: number; accuracy: number; speed?: number | null; timestamp: number }): void => { dbState.driver_locations.push({ id: `loc_${crypto.randomUUID()}`, ...record }); if (dbState.driver_locations.length > 5000) dbState.driver_locations = dbState.driver_locations.slice(-3000); saveDatabaseSync(); },
  ...
  getFoodCategoriesByCompanyId: (companyId: string): FoodCategory[] => (dbState.food_categories || []).filter((c) => c.companyId === companyId).sort((a, b) => { const orderA = a.displayOrder ?? a.sortOrder ?? 999; const orderB = b.displayOrder ?? b.sortOrder ?? 999; if (orderA !== orderB) return orderA - orderB; return (a.createdAt || 0) - (b.createdAt || 0); }),
  ...
  createWithdrawalRequest: (req: WithdrawalRequest): WithdrawalRequest => { if (!dbState.withdrawal_requests) dbState.withdrawal_requests = []; dbState.withdrawal_requests.unshift(req); saveDatabaseSync(); return req; },
  getWithdrawalRequest: (id: string): WithdrawalRequest | null => (dbState.withdrawal_requests || []).find((r) => r.id === id) || null,
  updateWithdrawalRequest: (id: string, updates: Partial<WithdrawalRequest>): WithdrawalRequest | null => { if (!dbState.withdrawal_requests) return null; const idx = dbState.withdrawal_requests.findIndex((r) => r.id === id); if (idx === -1) return null; dbState.withdrawal_requests[idx] = { ...dbState.withdrawal_requests[idx], ...updates, updatedAt: Date.now() }; saveDatabaseSync(); return dbState.withdrawal_requests[idx]; },
  getWithdrawalRequestsByCompany: (companyId: string): WithdrawalRequest[] => (dbState.withdrawal_requests || []).filter((r) => r.companyId === companyId),
  getAllWithdrawalRequests: (): WithdrawalRequest[] => dbState.withdrawal_requests || [],
