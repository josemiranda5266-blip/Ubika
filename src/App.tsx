...
export default function App() {
  ...
  const [publicFoodCompanyId, setPublicFoodCompanyId] = useState<string | undefined>();
  const [publicFoodOrderId, setPublicFoodOrderId] = useState<string | undefined>();
  ...
  useEffect(() => {
    const checkPublicRoute = () => {
      ...
      if (hash === '#food' || hash === '#food/admin' || hash.startsWith('#food/company/') || hash.startsWith('#food/order/')) {
        if (hash.startsWith('#food/company/')) setPublicFoodCompanyId(hash.replace('#food/company/', '') || undefined);
        if (hash.startsWith('#food/order/')) setPublicFoodOrderId(hash.replace('#food/order/', '') || undefined);
        setPublicMode('food');
        return;
      }
  ...
  if (publicMode === 'food' && !authToken) {
    return <UbikaFoodApp initialCompanyId={publicFoodCompanyId} initialOrderId={publicFoodOrderId} initialViewMode="customer" />;
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      <header className="bg-white border-b border-slate-200 px-4 sm:px-8 py-3 flex items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div><div className="text-xl font-black tracking-tighter">UBIKA</div><div className="text-[10px] font-black uppercase tracking-wider text-slate-400">Plataforma unificada</div></div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block"><div className="text-xs font-black text-slate-800 flex items-center justify-end gap-1"><User className="w-3.5 h-3.5 text-slate-400" />{currentUser.name}</div><div className="text-[9px] font-black uppercase text-orange-500 tracking-wider">{currentUser.role}</div></div>
