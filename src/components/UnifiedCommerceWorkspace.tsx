...
import { Store, Utensils, Truck, ChefHat, Users, ShieldCheck } from 'lucide-react';
...
export const UnifiedCommerceWorkspace: React.FC<UnifiedCommerceWorkspaceProps> = ({ user, onOpenCustomerLink }) => {
  const initialView = useMemo<WorkspaceView>(() => {
    ...
    if (user.role === 'SUPER_ADMIN') return 'control';
    return 'commerce';
  }, [user.role]);
  ...
  const navigation = [
    user.role === 'SUPER_ADMIN' && { id: 'control' as const, label: 'Control', icon: ShieldCheck },
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-4 shrink-0">
        <div>
          <div className="flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-orange-500" /><span className="text-lg font-black tracking-tight">UBIKA</span><span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Workspace</span></div>
        ...
        <nav className="flex items-center gap-1 overflow-x-auto max-w-[70vw]">
          {navigation.map(({ id, label, icon: Icon }) => (
            <button key={id} type="button" onClick={() => setView(id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-black whitespace-nowrap transition-all ${view === id ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-100'}`}><Icon className="w-3.5 h-3.5" />{label}</button>
          ))}
        </nav>
      </header>

      <main className="flex-1 min-h-0 overflow-auto">
        {view === 'control' && user.role === 'SUPER_ADMIN' && <UbikaControl onOpenCustomerLink={onOpenCustomerLink} />}
        ...
        {view === 'mozo' && (
          <div className="p-6 max-w-5xl mx-auto w-full"><div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm"><div className="flex items-center gap-3 mb-3"><Users className="w-6 h-6 text-orange-500" /><h1 className="text-xl font-black">Salón y mesas</h1></div><p className="text-sm text-slate-600 leading-6">El rol MOZO ya forma parte de la autorización de UBIKA. Este módulo será la fuente única de mesas y comandas: mesa → pedido → cocina → cuenta → pago. No se agregará persistencia ficticia; el modelo de mesas/comandas se implementará en el siguiente bloque.</p></div></div>
        )}
      </main>
    </div>
  );
